...
public class SetBankAccountDAO {
	public AccountBeans executeBankAccount(int changeBankAccount, AccountBeans accountBeans) {
		try {
			Class.forName("com.mysql.cj.jdbc.Driver");
			try(Connection conn = GetConnection.getConnection()) {
			  String sql1 = "UPDATE ACCOUNT SET BANK_ACCOUNT=? WHERE ID=?";
			  PreparedStatement tStmt = conn.prepareStatement(sql1);
			  tStmt.setInt(1, accountBeans.getBankAccount()+changeBankAccount);
