			...
			<div class="header">
				<div class="wrapper">
					...
					<div class="right">
						<p>
							<% if (account != null) { %>
								ようこそ、<%= account.getName() %> さん（<a href="/FINDs/MyPageServlet">マイページ</a>）<br>
								現在のポイント：<%= account.getPoints() %>P<br>
								現在の口座残高：<%= account.getBankAccount() %>円　
								<a href="/FINDs/LoginServlet?action=logout"><button>ログアウト</button></a>
							<% } else { %>
								ようこそ、ゲストさん <a href="/FINDs/LoginServlet"><button>ログイン</button></a>
							<% } %>
						</p>
					</div>
				</div>
				<ul class="menu-list">
					<li><a href="/FINDs/IndexServlet">ＴＯＰ</a></li>		
					<li><a href="/FINDs/ItemListServlet">FINDsShop</a></li>
				</ul>
			</div>
		</header>
