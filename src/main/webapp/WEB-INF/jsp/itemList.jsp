			...
			<c:forEach var="item" items="${ itemList }">
				<div class="item">
					<div class="item-inner">
						<a class="item-link" href="/FINDs/ItemDetailsServlet?ID=${ item.itemId }">
		...
		<div class="pagenation">
			<c:if test="${ pageNo != 1 }">
				<span class="left-allow allow">
					<a href="/FINDs/ItemListServlet?page=${ pageNo - 1 }"><div class="arrow-left icon"></div></a>
				</span>
			</c:if>
				<div class=page-link>
					<c:choose>
						<c:when test="${ maxPage <= 7 }">
							<c:forEach var="i" begin="1" end="${ maxPage }" step="1">
								<c:choose>
									...
									<c:otherwise>
										<a href="/FINDs/ItemListServlet?page=${ i }">
						...
						<c:when test="${ pageNo < 5  }">
							...
							<a href="/FINDs/ItemListServlet?page=${ maxPage }"><span class="page-number">${ maxPage }</span></a>
						</c:when>
						<c:when test="${ pageNo + 4 > maxPage }">
							<a href="/FINDs/ItemListServlet?page=1"><span class="page-number">1</span></a>
							<span class="page-number">...</span>
			...
			<c:if test="${ pageNo != maxPage }">
				<span class="right-allow allow">
					<a href="/FINDs/ItemListServlet?page=${ pageNo + 1 }"><div class="arrow-right icon"></div></a>
				</span>
			</c:if>
		</div>
